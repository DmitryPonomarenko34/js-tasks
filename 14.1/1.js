...
function CleanerRobot(
    ...
    this.stop = (delay) => {
        this.clean.bind(clearTimeout(timerId));
        return console.log(`Спустя ${(delay / 1000)} секунду: Уборка завершена досрочно. Осталось заряда батареи: ${energy} */`);
    }

    const lostEnergy = () => energy -= (getCleaningTime() * ENERGY_CONSUMPTION);
    lostEnergy.call(onReady);
...
setTimeout(() => {
    cleanerRobot.stop(delay); /* Спустя 1 секунду: Уборка завершена досрочно. Осталось заряда батареи: 45.5. */
}, delay = 1000);
